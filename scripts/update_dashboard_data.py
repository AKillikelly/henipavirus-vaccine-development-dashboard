...
The updater intentionally separates *curated maturity staging* from *automated source
monitoring*. It can safely refresh source availability, page titles, clinical-trial
registry metadata, and review flags on a GitHub Actions schedule without silently
promoting a vaccine candidate to a new stage.
"""
...
def limited_get(url: str, *, timeout: int, max_bytes: int, user_agent: str) -> dict[str, Any]:
    ...
    try:
        with requests.get(url, timeout=timeout, allow_redirects=True, headers=headers, stream=True) as response:
            ...
            result["title"] = extract_html_title(content, result.get("content_type"))
    except Exception as exc:  # noqa: BLE001 - report source failures without crashing the whole update.
...
def normalized_record(record: dict[str, Any]) -> dict[str, Any]:
    out = {key: record.get(key, "") for key in REQUIRED_RECORD_FIELDS if key != "sources"}
    ...
    out["is_gap"] = safe_bool(record.get("is_gap"))
    out["is_clinical"] = safe_bool(record.get("is_clinical"))
    out["lineage_or_scope"] = record.get("lineage_or_scope", "")
    out["sources"] = [s for s in (record.get("sources") or []) if isinstance(s, dict) and s.get("url")]
    out["registry_watch"] = [w for w in (record.get("registry_watch") or []) if isinstance(w, dict)]
    out["curation_lock"] = safe_bool(record.get("curation_lock", True))
    return out
...
def flags_for_records(
    ...
    for record in records:
        ...
        for watch in record.get("registry_watch") or []:
            ...
            if overall in terminal and record.get("stage_key") in {"phase1", "phase2"}:
...
def attach_record_runtime_fields(
    records: list[dict[str, Any]], registry_statuses: list[dict[str, Any]], flags: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    by_registry = {(item.get("type"), item.get("id")): item for item in registry_statuses}
    ...
    for record in records:
        ...
        for watch in rec.get("registry_watch") or []:
            ...
            if status:
                statuses.append(status)
        rec["registry_statuses"] = statuses
...
def write_outputs(
    ...
    *,
    ...
    enriched = attach_record_runtime_fields(records, registry_statuses, flags)
    ...
    payload = {
        ...
        "dashboard_title": cfg.get("dashboard_title", "Henipavirus Vaccine Development Pathway Dashboard"),
            ...
            writer.writerow(
                {
                    ...
                    "virus": rec.get("virus"),
                    "lineage_or_scope": rec.get("lineage_or_scope"),
                    "stage": rec.get("stage"),
                    "stage_key": rec.get("stage_key"),
                    "stage_order": rec.get("stage_order"),
                    "platform": rec.get("platform"),
                    "sponsor_or_steward": rec.get("sponsor_or_steward"),
                    "setting": rec.get("setting"),
                    "status_summary": rec.get("status_summary"),
                    "next_milestone_or_gap": rec.get("next_milestone_or_gap"),
                    "evidence_class": rec.get("evidence_class"),
                    "is_gap": rec.get("is_gap"),
                    "is_clinical": rec.get("is_clinical"),
                    "source_titles": titles,
                    "source_urls": urls,
                    "registry_status_summary": registry_summary(rec),
                    "review_flags": " | ".join(flag.get("message", "") for flag in rec.get("review_flags", [])),
    ...
    (out_dir / "last_update.json").write_text(json.dumps(last_update, indent=2) + "\n", encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    ...
    user_agent = str(http_rules.get("user_agent") or "HenipavirusDashboardBot/1.0")
    ...
    flags = flags_for_records(records, source_checks, registry_statuses, rules)
    write_outputs(
        cfg,
        rules,
        records,
    ...
    if args.fail_on_review_flags and flags:
        ...
        return 2
    print(f"Wrote dashboard data to {args.out} with {len(records)} records, {len(source_checks)} source checks, {len(flags)} flags.")
    return 0
